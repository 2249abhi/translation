...
class Manager{
    ...
    public function importTranslations( $replace = false, $base = null )
    {
        ...
        if(is_dir(base_path().DIRECTORY_SEPARATOR.'modules')) {

            $modules_list = $this->files->directories(base_path().DIRECTORY_SEPARATOR.'modules');

                foreach ($modules_list as $key => $modulePath) {
                    if(is_dir(trim($modulePath.DIRECTORY_SEPARATOR.'resources'.DIRECTORY_SEPARATOR.'lang'))) {
                        $module_langs[] = $this->files->directories(trim($modulePath.DIRECTORY_SEPARATOR.'resources'.DIRECTORY_SEPARATOR.'lang'));     
                    }
            } 

            if(!empty($module_langs)) {
                foreach($module_langs as $tmp) {foreach($tmp as $val) {
                        $paths[] = $val;            
                    }
                }

                $langPaths = array_merge($paths, $this->files->directories($this->app['path.lang']));    

            } else {
                $langPaths = $this->files->directories($this->app['path.lang']);
            }
        ...
        foreach ( $langPaths as $langPath ) {
            ...
            $Mname = strtolower(basename(str_replace(DIRECTORY_SEPARATOR.'resources'.DIRECTORY_SEPARATOR.'lang'.DIRECTORY_SEPARATOR.$locale, '', $langPath)));
            ...
            foreach ( $this->files->allfiles( $langPath ) as $file ) {
                ...
                if ( !$vendor ) {
                    if(strpos($langPath, 'modules')) {
                        $translations = \Lang::get($Mgroup, [], $locale);
                    } else {
                ...
                if ( $translations && is_array( $translations ) ) {
                    foreach ( array_dot( $translations ) as $key => $value ) {
                        if(strpos($langPath, 'modules')) {
                            $importedTranslation = $this->importTranslation( $key, $value, $locale, $Mgroup, $replace );
                        } else {
            ...
            if (!in_array($group, $this->config['exclude_groups'])) {
                ...
                foreach ($tree as $locale => $groups) {
                    if (isset($groups[$group])) {
                        ...
                        if(strpos($group, '::')) {
                            $tpath = explode('::', $group);
                            $path = base_path().'/Modules/'.$tpath[0].'/resources/lang/'.$locale.'/'.$tpath[1].'.php';
                        } else {
                            $path = $path . '/' . $group . '.php';    
                        }
