    ...
    <script>
        jQuery(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                var group = $(this).val();
                if (group) {
                    window.location.href = '<?= action('\Translation\Controller@getView') ?>/'+$(this).val();
                } else {
                    window.location.href = '<?= action('\Translation\Controller@getIndex') ?>';
                }
            });
            ...
            $("a.delete-key").click(function(event){
              ...
              var key = $(this).attr('key');
                ...
                var r = confirm("Are you sure you want to delete the translations for '"+ key +"'");
                if (r == true) {
...
<div style="width: 80%; margin: auto;">
    ...
    <div class="alert alert-success success-import" style="display:none;">
        <p>Done importing, processed <strong class="counter">N</strong> items! Please Wait!</p>
    </div>
    ...
    <p>
        <?php if(!isset($group)) : ?>
        <form class="form-inline form-import" method="POST" action="<?= action('\Translation\Controller@postImport') ?>" data-remote="true" role="form">
        ...
        <form class="form-inline form-find" method="POST" action="<?= action('\Translation\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <p></p>
        ...
        <?php endif; ?>
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?= action('\Translation\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations group '<?= $group ?>? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Publishing.." >Publish translations</button>
                <a href="<?= action('\Translation\Controller@getIndex') ?>" class="btn btn-default">Back</a>
            </form>
        <?php endif; ?>
    </p>
    <form role="form">
    ...
    <?php if($group): ?>
        <form action="<?= action('\Translation\Controller@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
    ...
    <table class="table">
        ...
        <?php foreach($translations as $key => $translation): ?>
            <tr id="<?= $key ?>">
                ...
                <?php if($deleteEnabled): ?>
                    <td>
                        <a href="<?= action('\Translation\Controller@postDelete', [$group, $key]) ?>" key="<?= $key ?>" class="delete-key"><span class="glyphicon glyphicon-trash"></span></a>
                    </td>
                <?php endif; ?>
            </tr>
        <?php endforeach; ?>
